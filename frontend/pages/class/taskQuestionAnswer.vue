...
<script>
	...
	export default {
		...
		methods: {
			...
			randTen() {
				...
				uni.request({
					...
					success(res) {
						...
						self.questionBanks = res.data.data;
					}
				});
			},
			switchTabToClassify(index) {
				if (this.current == this.questionBanks.length-1) this.controlShow = false;
				console.log(index)
			...
			gotoNav() {
				if (this.current >= this.questionBanks.length) {
					console.log("要进行跳转拉")
					...
					uni.request({
						url: this.serverUrl + '/task/saveResult', //仅为示例，并非真实接口地址。
						data: {
					...
					// uni.navigateTo({
					// 	url: `/pages/class/taskQuestionResult?score=${score}&list=${list}`
					// });
...
<style>
	@import url("taskQuestionAnswer.css");
</style>
